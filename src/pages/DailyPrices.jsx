...
const DailyPrices = () => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-10 max-w-5xl mx-auto">
      <div className="bg-white dark:bg-emerald-900/20 rounded-[3rem] shadow-2xl border border-emerald-100 dark:border-emerald-800 p-10">
        ...
        <div className="flex justify-between items-center mb-10 border-b-4 border-emerald-600 pb-6">
          <div className="space-y-1">
            <h1 className="text-2xl font-black text-emerald-700 dark:text-emerald-400">OFFICIAL NIRKH NAMA</h1>
            <div className="flex items-center gap-2 text-slate-400 text-xs font-bold uppercase tracking-widest">
              <Calendar size={14} /> {new Date().toLocaleDateString()}
            </div>
          </div>
          <div className="text-right">
            <h2 className="text-3xl font-black dark:text-white" dir="rtl">ضلع چارسدہ نرخنامہ</h2>
            <p className="text-[10px] text-emerald-600 font-bold">FOOD DEPARTMENT CONTROL PANEL</p>
          </div>
        </div>
        ...
        <div className="space-y-3">
          {prices.map((item, i) => (
            ...
            >
              <input type="number" defaultValue={item.g2} className="w-full bg-white dark:bg-emerald-900 p-3 rounded-xl text-center font-bold text-emerald-600 outline-none focus:ring-2 focus:ring-emerald-500" />
              <input type="number" defaultValue={item.g1} className="w-full bg-white dark:bg-emerald-900 p-3 rounded-xl text-center font-bold text-emerald-600 outline-none focus:ring-2 focus:ring-emerald-500" />
              <div className="col-span-2 text-right pr-4">
                <p className="font-black text-slate-800 dark:text-white text-lg">{item.urdu}</p>
